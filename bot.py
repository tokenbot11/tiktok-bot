...
TOKEN = "7512316456:AAGYLRPdQxTXuswDp2ISeqXSpSBfIcEMx-U"  # از متغیر محیطی در Render استفاده کنید
ADMIN_ID = "7109901365"
CHANNELS = ["@downloaderbe", "@Drops1Drop"]

async def shorten_url(long_url):
    # در صورت نیاز به Bitly یا سرویس دیگر، توکن و API را اضافه کنید
    return long_url  # موقتاً کوتاه‌سازی غیرفعال شده
...
async def fetch_youtube_info(url):
    async with aiohttp.ClientSession() as session:
        try:
            ...
            raise Exception(f"Error: {str(e)}. Check the link or try a different VPN.")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == "instagram" and "instagram.com" in url:
        ...
        try:
            ...
            await update.message.reply_text(f"❌ خطا: {e}", reply_markup=get_after_download_menu())
