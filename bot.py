...
async def fetch_youtube_info(url):
    ...
    payload = {"url": url, "q_auto": "0"}
    async with aiohttp.ClientSession(headers=headers) as session:
        try:
            logger.info(f"Fetching YouTube info for URL: {url}")
            async with session.post("https://yt1s.com/api/youtube/search", data=payload, timeout=aiohttp.ClientTimeout(total=15)) as response:
                text = await response.text()
                logger.info(f"Response from yt1s: {text[:500]}")
                data = json.loads(text)
                if data.get("status") != "success":
                    raise Exception("Error fetching video info from yt1s")
                
                qualities = []
                vid = data.get("vid", "unknown")
                for item in data.get("links", {}).get("mp4", []):
                    quality = item.get("q", "unknown")
                    if quality in ["360p", "480p", "720p", "1080p"]:
                        qualities.append({"q": quality, "k": item["k"]})
                
                if not qualities:
                    raise Exception("No quality options found")
                
                keyboard = [[InlineKeyboardButton(f"🎥 {info['q']}", callback_data=f"yt1s_{vid}_{info['k']}")] 
...
async def get_youtube_download_link(vid, k):
    ...
    async with aiohttp.ClientSession(headers=headers) as session:
        try:
            ...
            async with session.post("https://yt1s.com/api/youtube/convert", data=payload, timeout=aiohttp.ClientTimeout(total=15)) as response:
