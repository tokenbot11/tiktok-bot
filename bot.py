...
async def fetch_youtube_info(url):
    ...
    async with aiohttp.ClientSession(headers=headers) as session:
        try:
            ...
            async with session.post("https://ssyoutube.com/api/convert", data=payload, timeout=aiohttp.ClientTimeout(total=15)) as response:
                text = await response.text()
                logger.info(f"Response from ssyoutube: {text[:500]}")
                data = json.loads(text)
                if "url" not in data or not data["url"]:
                    raise Exception("Error fetching video info from ssyoutube")
                ...
                keyboard = [[InlineKeyboardButton(f"🎥 {info['q']}", callback_data=f"ssyoutube_{vid}_{info['q']}")] 
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif data.startswith("ssyoutube_"):
        _, vid, q = data.split("_", 2)
        await query.edit_message_text("🎥 در حال آماده‌سازی لینک...")
        try:
            download_url = context.user_data.get(f"ssyoutube_{vid}_{q}")
            if not download_url:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == "instagram" and "instagram.com" in url:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post("https://ssyoutube.com/api/convert", data={"id": url}) as response:
                    data = await response.json()
                    for item in data["url"]:
                        quality = item.get("quality", "unknown")
                        if quality in ["360p", "480p", "720p", "1080p"]:
                            context.user_data[f"ssyoutube_{data['id']}_{quality}"] = item["url"]
