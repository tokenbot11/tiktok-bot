...
async def fetch_youtube_info(url):
    ...
    async with aiohttp.ClientSession(headers=headers) as session:
        try:
            ...
            async with session.post("https://api.savefrom.net/api/convert", data={"url": url}, timeout=aiohttp.ClientTimeout(total=15)) as response:
                text = await response.text()
                logger.info(f"Response from SaveFrom: {text[:500]}")
                data = json.loads(text)
                if "url" not in data or not data["url"]:
                    raise Exception("Error fetching video info from SaveFrom")
                
                qualities = []
                for item in data["url"]:
                    quality = item.get("quality", "unknown")
                    if quality in ["360p", "480p", "720p", "1080p"]:
                        qualities.append({"q": quality, "url": item["url"]})
                
                if not qualities:
                    raise Exception("No quality options found")
                
                vid = data.get("id", "unknown")
                keyboard = [[InlineKeyboardButton(f"🎥 {info['q']}", callback_data=f"savefrom_{vid}_{info['q']}")] 
...
async def get_youtube_download_link(vid, quality):
    return quality  # لینک مستقیم از fetch_youtube_info استفاده می‌شود
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif data.startswith("savefrom_"):
        _, vid, q = data.split("_", 2)
        await query.edit_message_text("🎥 در حال آماده‌سازی لینک...")
        try:
            download_url = context.user_data.get(f"savefrom_{vid}_{q}")
            if not download_url:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == "instagram" and "instagram.com" in url:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post("https://api.savefrom.net/api/convert", data={"url": url}) as response:
                    data = await response.json()
                    for item in data["url"]:
                        quality = item.get("quality", "unknown")
                        if quality in ["360p", "480p", "720p", "1080p"]:
                            context.user_data[f"savefrom_{data['id']}_{quality}"] = item["url"]
