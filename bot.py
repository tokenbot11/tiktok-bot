...
async def fetch_youtube_info(url):
    ydl_opts = {
        "quiet": True,
        "format": "bestvideo+bestaudio/best",  # بهترین کیفیت ویدیو و صدا
        "noplaylist": True,  # فقط یک ویدیو، نه پلی‌لیست
        "listformats": True,  # برای گرفتن لیست کیفیت‌ها
    }
    try:
